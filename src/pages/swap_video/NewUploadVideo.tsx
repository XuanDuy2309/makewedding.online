...
const NewUploadVideo = () => {
    ...
    useEffect(() => {
        axios.get(`https://api.funface.online/images/${user.id_user}?type=video`, {
            headers: {
    ...
    const handleGenerate = async () => {
            ...
            if (original_Image_1) {
                ...
                req_post_img_1 = axios.post(`https://api.funface.online/upload-gensk/${userData.id_user}?type=src_nam`, image_1_formData, {
                    headers: {
            ...
            if (src_res_1 != null) {
                try{
                   setIsLoading(true)
                   const response = await axios.get(`https://video.funface.online/getdata/genvideo/swap/imagevid/wedding?device_them_su_kien=${userData.device_register}&ip_them_su_kien=${userData.ip_register}&id_user=${userData.id_user}&src_img=${src_res_1}&src_vid_path=${id}`, {
                        headers: {
                            'Authorization': 'Bearer ' + localStorage.getItem('accessToken'),
                        }
                    })
