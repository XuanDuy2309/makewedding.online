...
const Editor = () => {
    ...
    axios
      ...
      .then(() => {
        user.link_avatar.includes("/var/www/build_futurelove/")
          ? setAvatarLink(
              `https://photo.gachmen.org/${user.link_avatar.replace(
  ...
  useEffect(() => {
    axios
      .get(`https://api.funface.online/images/${id}?type=video`, {
        headers: {
  ...
  const handleSaveForm = () => {
    if (avatarUploadFile) {
      ...
      axios
        .post(
          `https://api.fuface.online/upload-gensk/${user.id_user}?type=src_vid`,
          formData,
          {
            headers: {
        ...
        .then((res) => {
          if (res.status.toString().startsWith("2")) {
            axios
              .post(
                `https://api.fuface.online/changeavatar/${user.id_user}`,
                {
                  link_img: res.data,
                  check_img: "upload",
                },
      ...
      axios
        .post(
          `https://api.funface.online/changeavatar/${user.id_user}`,
          {
            link_img: chosenImg,
            check_img: "upload",
          },
