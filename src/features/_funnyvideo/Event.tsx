...
const EventVideo = () =>{
    ...
    const [oldVid,setOldVid] = useState("");
    const {id_user, id_sukien} = useParams()
    const fetchUser = () =>{
        axios.get(`https://api.funface.online/profile/${id_user}`)
        .then((res) => setUser1(res.data))
    }
    const fetchEvent = () =>{
        axios.get(`https://api.funface.online/get/list_video/id_video_swap_all_id_sk?id_user=${id_user}&id_sk=${id_sukien}`)
    ...
    const fetchImg = async () => {
        try {
          const res = await axios.get(`https://api.funface.online/get/list_video/all_video_wedding_template`)
    ...
    const fetchVidOld= async () =>{
        try {
            const {data} = await axios.get(`https://api.funface.online/get/list_video/video_wedding_detail?id=2`);
            setOldVid(data)
            ...
            <div className="pb-[70px] pt-[40px]">
                ...
                <div className="mx-auto">       
                    ...
                    <div className="w-[80px] h-[80px]">
                        {
                              user1.link_avatar?.includes("https://photo.gachmen.org")
                              ?
                              ...
                              (
                                ...
                                <img className="h-full w-full object-cover" src={`https://photo.gachmen.org/${user1.link_avatar?.replace("/var/www/build_futurelove/","")}`} alt="undefind image" />
                              :
                    ...
                    <div className="md:h-[300px] md:w-[300px] h-[150px] w-[150px] mx-auto">
                        <img src={listTemp[0]?.link_image.includes('/var/www/build_futurelove/')?`https://photo.gachmen.org/${listTemp[0]?.link_image.replace("/var/www/build_futurelove/","")}`:listTemp[0]?.link_image} className="h-[150px] w-[150px] md:h-[300px] md:w-[300px] border-4 border-[#d2f6ff] object-cover" alt="111" />
                    </div>
                    ...
                    <div className="md:w-[300px] md:h-[400px] w-[150px] h-[200px]">
                        <video src={oldVid} className="w-full h-full object-contain border-4 border-[#dff9ff]" controls/>
                    </div>
